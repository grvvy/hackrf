...
class TestFixedValue(unittest.TestCase):
    ...
    def test_round(self):
        ...
        # Round with width extension.
        self.assertFixedEqual(
            fixed.Const(-0.015625).round(0, clip=False),
            fixed.Const(0, fixed.SQ(2, 0))
        )

        self.assertFixedEqual(
            fixed.Const(-0.65625).round(0, clip=False),
            fixed.Const(-1, fixed.SQ(2, 0))
        )

        self.assertFixedEqual(
            fixed.Const(0.65625, fixed.SQ(1,7)).round(0, clip=False),
            fixed.Const(1, fixed.SQ(2, 0))
        )

        self.assertFixedEqual(
            fixed.Const(0.15625, fixed.SQ(1,7)).round(0, clip=False),
            fixed.Const(0, fixed.SQ(2, 0))
        )

        self.assertFixedEqual(
            fixed.Const(0.99609375, fixed.SQ(1,8)).round(7, clip=False),
            fixed.Const(1, fixed.SQ(2, 7))
        )

        # Round with saturation for avoiding rounding-induced overflow.
        self.assertFixedEqual(
            fixed.Const(-0.016, fixed.SQ(1, 17)).round(7),
            fixed.Const(-0.015625, fixed.SQ(1, 7))
        )

        self.assertFixedEqual(
            fixed.Const(0.99609375, fixed.SQ(1,8)).round(7),
            fixed.Const(0.9921875, fixed.SQ(1, 7))
        )

        self.assertFixedEqual(
            fixed.Const(-0.99609375, fixed.SQ(1,8)).round(7),
            fixed.Const(-1, fixed.SQ(1, 7))
        )

        self.assertFixedEqual(
            fixed.Const(0.015625, fixed.SQ(1,7)).round(0),
            fixed.Const(0, fixed.SQ(1, 0))
        )

        self.assertFixedEqual(
            fixed.Const(-0.1625, fixed.SQ(1,7)).round(0),
            fixed.Const(0, fixed.SQ(1, 0))
        )

        self.assertFixedEqual(
            fixed.Const(-0.6625, fixed.SQ(1,7)).round(0),
            fixed.Const(-1, fixed.SQ(1, 0))
        )
    ...
    def test_saturate(self):
        ...
        self.assertFixedEqual(
            fixed.Const(-0.015625, fixed.SQ(2, 7)).saturate(fixed.SQ(1, 7)),
            fixed.Const(-0.015625, fixed.SQ(1, 7))
        )
